...
Status VehicleStateProvider::Update(
  ...
  if (!ConstructExceptLinearVelocity(localization)) {
    ...
    return Status(controller::ErrorCode::LOCALIZATION_ERROR, msg);
  }
...
bool VehicleStateProvider::ConstructExceptLinearVelocity(
  ...
  if (!localization.has_pose()) {
    LOG(ERROR) << "Invalid localization input.";
    return false;
  }

  // skip localization update when it is in use_navigation_mode.
  if (FLAGS_use_navigation_mode) {
    LOG(INFO) << "Skip localization update when it is in use_navigation_mode.";
    return true;
  }
  ...
  if (FLAGS_enable_map_reference_unify) {
    ...
    if (!localization.pose().has_linear_acceleration_vrf()) {
      LOG(ERROR) << "localization.pose().has_linear_acceleration_vrf() must be "
    ...
    if (!localization.pose().has_angular_velocity()) {
      LOG(ERROR) << "localization.pose() has no angular velocity.";
      return false;
    }
    vehicle_state_.set_angular_velocity(
    ...
    if (!localization.pose().has_linear_acceleration()) {
      LOG(ERROR) << "localization.pose() has no linear acceleration.";
      return false;
    }
    vehicle_state_.set_linear_acceleration(
...
const controller::Pose &VehicleStateProvider::pose() const {
  return vehicle_state_.pose();
}

const controller::Pose &VehicleStateProvider::original_pose() const {
...
const vehicle_state::VehicleState &VehicleStateProvider::vehicle_state() const {
