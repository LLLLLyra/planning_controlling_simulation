#ifndef _MAC_CONTROLLER_H_
#define _MAC_CONTROLLER_H_

#include <vector>

#include "Eigen/Core"
#include "common/proto/mrac_conf.pb.h"
...
class MracController {
 public:
   ...
   */
  void Init(const controller::MracConf &mrac_conf,
            const vehicle::LatencyParam &latency_param, const double dt);
   ...
   */
  Status SetReferenceModel(const controller::MracConf &mrac_conf);
   ...
   * @return Status parameter initialization status
   */
  Status SetAdaptionModel(const controller::MracConf &mrac_conf);
   ...
   */
  void EstimateInitialGains(const vehicle::LatencyParam &latency_param);
