...
void MracController::Init(const controller::MracConf &mrac_conf,
...
Status MracController::SetReferenceModel(
    const controller::MracConf &mrac_conf) {
  ...
  if (((mrac_conf.reference_time_constant() < Epsilon && model_order_ == 1)) ||
    ...
    const auto error_msg =
        ...
        std::to_string(mrac_conf.reference_time_constant()) +
        "and natural frequency parameter: " +
        std::to_string(mrac_conf.reference_natural_frequency()) +
        " in configuration file are not reasonable with respect to the " +
        "reference model order: " + std::to_string(model_order_);
    LOG(ERROR) << error_msg;
...
Status MracController::SetAdaptionModel(const controller::MracConf &mrac_conf) {
  ...
  if (p_size != model_order_ * model_order_ || x_size > model_order_ ||
    ...
    const auto error_msg =
        ...
        ", state gain number: " + std::to_string(x_size) +
        ", and anti-windup compensation gain number: " +
        std::to_string(aw_size) +
        " in configuration file do not match the model number: " +
        std::to_string(model_order_);
    LOG(ERROR) << error_msg;
    return Status(ErrorCode::CONTROL_INIT_ERROR, error_msg);
  }
...
Status MracController::BuildReferenceModel() {
  if (model_order_ > 2) {
    const auto error_msg = "mrac controller error: reference model order " +
                           std::to_string(model_order_) +
...
Status MracController::BuildAdaptionModel() {
  if (model_order_ > 2) {
    const auto error_msg = "mrac controller error: adaption model order " +
                           std::to_string(model_order_) +
                           " is beyond the designed range";
    LOG(ERROR) << error_msg;
    return Status(ErrorCode::CONTROL_INIT_ERROR, error_msg);
...
void MracController::EstimateInitialGains(const LatencyParam &latency_param) {
  ...
  if (model_order_ == 1 &&
    ...
    gain_state_adaption_init_.col(0) =
        (PseudoInverse<double, 2, 1>(matrix_b_estimate) *
         (matrix_a_reference_ - matrix_a_estimate))
            .transpose();
    gain_input_adaption_init_.col(0) =
        (PseudoInverse<double, 2, 1>(matrix_b_estimate) * matrix_b_reference_)
            .transpose();
  } else {
    LOG(INFO) << "No pre-known actuation dynamics; the initial states of the "
...
void MracController::SetInitialReferenceState(
  ...
  if (state_reference_init.rows() != model_order_ ||
    ...
    LOG(INFO) << "failed to set the initial reference states, due to the given "
                 "state size: "
...
void MracController::SetInitialActionState(const Matrix &state_action_init) {
  if (state_action_init.rows() != model_order_ ||
    ...
    LOG(INFO) << "failed to set the initial action states, due to the given "
                 "state size: "
