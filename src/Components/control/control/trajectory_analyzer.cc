...
double PointDistanceSquare(const points::TrajectoryPoint &point, const double x,
                           const double y) {
...
points::PathPoint TrajectoryPointToPathPoint(
    const points::TrajectoryPoint &point) {
  if (point.has_path_point()) {
    return point.path_point();
  } else {
    return points::PathPoint();
  }
}
...
points::PathPoint TrajectoryAnalyzer::QueryMatchedPathPoint(
...
void TrajectoryAnalyzer::ToTrajectoryFrame(const double x, const double y,
                                           ...
                                           const points::PathPoint &ref_point,
  ...
  *ptr_d_dot = v * sin_delta_theta;
  ...
  if (one_minus_kappa_r_d <= 0.0) {
    LOG(INFO) << "TrajectoryAnalyzer::ToTrajectoryFrame "
...
points::TrajectoryPoint TrajectoryAnalyzer::QueryNearestPointByAbsoluteTime(
    const double t) const {
  return QueryNearestPointByRelativeTime(t - header_time_);
}

points::TrajectoryPoint TrajectoryAnalyzer::QueryNearestPointByRelativeTime(
    const double t) const {
  auto func_comp = [](const points::TrajectoryPoint &point,
...
points::TrajectoryPoint TrajectoryAnalyzer::QueryNearestPointByPosition(
    const double x, const double y) const {
  size_t index_min = QueryNearestIndexByPosition(x, y);
  return trajectory_points_[index_min];
}

size_t TrajectoryAnalyzer::QueryNearestIndexByPosition(const double x,
...
size_t TrajectoryAnalyzer::QueryIndexByPosition(const double x, const double y,
                                                const double d) const {
  // Query the nearest
  size_t index_min = QueryNearestIndexByPosition(x, y);
  size_t index;
  const double target_distance = d * d;
  const double x_proj = trajectory_points_[index_min].path_point().x();
  const double y_proj = trajectory_points_[index_min].path_point().y();
  double current_distance = 0.0;
  for (index = index_min; index < trajectory_points_.size() - 1 &&
                          current_distance < target_distance;
       index++) {
    current_distance =
        PointDistanceSquare(trajectory_points_[index], x_proj, y_proj);
  }
  return index;
}

const std::vector<points::TrajectoryPoint>
    &TrajectoryAnalyzer::trajectory_points() const {
  return trajectory_points_;
}

points::PathPoint TrajectoryAnalyzer::FindMinDistancePoint(
    const points::TrajectoryPoint &p0, const points::TrajectoryPoint &p1,
    const double x, const double y) const {
  ...
  points::PathPoint p = p0.path_point();
  double s = GoldenSectionSearch(dist_square, p0.path_point().s(),
                                 p1.path_point().s());
  p.set_s(s);
  p.set_x(lerp(p0.path_point().x(), p0.path_point().s(), p1.path_point().x(),
               p1.path_point().s(), s));
  p.set_y(lerp(p0.path_point().y(), p0.path_point().s(), p1.path_point().y(),
               p1.path_point().s(), s));
  p.set_theta(slerp(p0.path_point().theta(), p0.path_point().s(),
                    p1.path_point().theta(), p1.path_point().s(), s));
  // approximate the curvature at the intermediate point
  p.set_kappa(lerp(p0.path_point().kappa(), p0.path_point().s(),
