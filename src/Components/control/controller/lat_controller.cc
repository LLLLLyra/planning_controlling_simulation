...
std::string GetLogFileName() {
  ...
  const std::string log_name =
      FLAGS_controller_log +
      std::string("steer_log_simple_optimal_%F_%H%M%S.csv");
...
LatController::LatController() : name_("LQR-based Lateral Controller") {
  if (FLAGS_enable_csv_debug) {
    steer_log_file_.open(GetLogFileName(), std::ofstream::out);
...
bool LatController::LoadControlConf(
  ...
  if (!control_conf) {
    LOG(ERROR) << "[LatController] control_conf == nullptr";
    return false;
  }
  ...
  if (ts_ <= 0.0) {
    LOG(ERROR) << "[LatController] Invalid control update interval.";
    return false;
  }
...
void LatController::ProcessLogs(const controller::SimpleLateralDebug *debug,
                                const canbus::Chassis *chassis) {
  char buffer[1000];
  memset(buffer, 0, sizeof(buffer));
  snprintf(buffer, 999,
           "%.6lf,%.6lf,%.6lf,%.6lf,%.6lf,%.6lf,%.6lf,%.6lf,%.6lf,%.6lf,%.6lf,%"
           ".6lf,%.6lf,%.6lf,%.6lf,%.6lf",
  ...
  LOG(INFO) << "Steer_Control_Detail: " << log_str;
}

void LatController::LogInitParameters() {
  LOG(INFO) << name_ << " begin.";
  LOG(INFO) << "[LatController parameters]"
            << " mass_: " << mass_ << ","
            << " iz_: " << iz_ << ","
            << " lf_: " << lf_ << ","
            << " lr_: " << lr_;
...
Status LatController::ComputeControlCommand(
  ...
  */
  ...
  if (enable_mrac_) {
    ...
    controller::MracDebug *mracdebug = debug->mutable_steer_mrac_debug();
...
void LatController::UpdateState(controller::SimpleLateralDebug *debug) {
  auto vehicle_state = injector_->vehicle_state();
  if (FLAGS_use_navigation_mode) {
...
void LatController::ComputeLateralErrors(
  ...
  if (FLAGS_query_time_nearest_point_only) {
    target_point = trajectory_analyzer.QueryNearestPointByAbsoluteTime(
        Timer::Now() + query_relative_time_);
  } else {
    if (FLAGS_use_navigation_mode &&
        !FLAGS_enable_navigation_mode_position_update) {
      target_point = trajectory_analyzer.QueryNearestPointByAbsoluteTime(
  ...
  LOG(INFO) << "match point information : " << target_point.ShortDebugString();
  ...
  if (injector_->vehicle_state()->gear() == canbus::Chassis::GEAR_REVERSE) {
    ...
    heading_error_feedback =
        NormalizeAngle(heading_error + target_point.path_point().theta() -
