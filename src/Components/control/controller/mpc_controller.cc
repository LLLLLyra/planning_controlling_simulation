...
std::string GetLogFileName() {
  ...
  const std::string log_name =
      FLAGS_controller_log + std::string("mpc_controller_%F_%H%M%S.csv");
...
bool MPCController::LoadControlConf(
  ...
  if (!control_conf) {
    LOG(ERROR) << "[MPCController] control_conf = nullptr";
    return false;
  }
  ...
  if (ts_ <= 0.0) {
    LOG(ERROR) << "[MPCController] Invalid control update interval.";
    return false;
  }
  ...
  if (std::isnan(steer_ratio_) || steer_ratio_ < kEpsilon) {
    LOG(ERROR) << "[MPCController] steer_ratio = 0";
    return false;
  }
  ...
  LOG(INFO) << "MPC conf loaded";
  return true;
}

void MPCController::ProcessLogs(const controller::ControlCommand *cmd,
  ...
  snprintf(
     ...
     %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %.6f, %d",
      ...
      debug->lateral_error_rate(), debug->curvature(), debug->steer_angle(),
      ...
      injector_->vehicle_state()->linear_velocity(), debug->station_reference(),
      debug->station_error(), debug->speed_reference(), debug->speed_error(),
      debug->acceleration_cmd_closeloop(), debug->acceleration_cmd(),
      debug->acceleration_lookup(), debug->speed_lookup(),
      debug->calibration_value(), cmd->throttle(), cmd->brake(),
      debug->is_full_stop());
  ...
  LOG(INFO) << "MPC_Control_Detail: " << log_str;
}

void MPCController::LogInitParameters() {
  LOG(INFO) << name_ << " begin.";
  LOG(INFO) << "[MPCController parameters]"
            << " mass_: " << mass_ << ","
            << " iz_: " << iz_ << ","
            << " lf_: " << lf_ << ","
            << " lr_: " << lr_;
...
Status MPCController::Init(std::shared_ptr<DependencyInjector> injector,
  ...
  if (basic_state_size_ != q_param_size) {
    const auto error_msg =
        "MPC controller error: matrix_q size: " + std::to_string(q_param_size) +
        " in parameter file not equal to basic_state_size_: " +
        std::to_string(basic_state_size_);
    LOG(ERROR) << error_msg;
  ...
  LogInitParameters();
  LOG(INFO) << "[MPCController] init done!";
  return Status::OK();
}
...
void MPCController::LoadMPCGainScheduler(
  ...
  LOG(INFO) << "MPC control gain scheduler loaded";
...
Status MPCController::ComputeControlCommand(
  ...
  controller::SimpleMPCDebug *debug =
      cmd->mutable_debug()->mutable_simple_mpc_debug();
  debug->Clear();
  ...
  double mpc_start_timestamp = Timer::Now();
  ...
  MpcOsqp mpc_osqp(matrix_ad_, matrix_bd_, matrix_q_updated_, matrix_r_updated_,
  ...
  if (!mpc_osqp.Solve(&control_cmd)) {
    LOG(ERROR) << "MPC OSQP solver failed";
  } else {
    LOG(INFO) << "MPC OSQP problem solved! ";
  ...
  double mpc_end_timestamp = Timer::Now();

  LOG(INFO) << "MPC core algorithm: calculation time is: "
...
void MPCController::UpdateState(controller::SimpleMPCDebug *debug) {
...
void MPCController::UpdateMatrix(controller::SimpleMPCDebug *debug) {
...
void MPCController::FeedforwardUpdate(controller::SimpleMPCDebug *debug) {
  const double v = injector_->vehicle_state()->linear_velocity();
  const double kv =
...
void MPCController::ComputeLateralErrors(
  ...
  const double delta_theta = NormalizeAngle(theta - debug->ref_heading());
...
void MPCController::ComputeLongitudinalErrors(
  ...
  const double current_control_time = Timer::Now();
  ...
  double heading_error = NormalizeAngle(injector_->vehicle_state()->heading() -
                                        matched_point.theta());
