...
HybridAStar::HybridAStar(
    const planning::PlannerOpenSpaceConfig& open_space_conf) {
...
bool HybridAStar::GenerateSpeedAcceleration(HybridAStarResult* result) {
  ...
  for (size_t i = 0; i + 1 < x_size; ++i) {
    double discrete_steer =
        NormalizeAngle(result->phi[i + 1] - result->phi[i]) *
...
bool HybridAStar::TrajectoryPartition(
  ...
  bool current_gear =
      std::abs(NormalizeAngle(tracking_angle - heading_angle)) < (M_PI_2);
  ...
  for (auto& result : *partitioned_result) {
    if (FLAGS_use_s_curve_speed_smooth) {
      if (!GenerateSCurveSpeedAcceleration(&result)) {
        LOG(ERROR) << "GenerateSCurveSpeedAcceleration fail";
        return false;
      }
    } else {
      if (!GenerateSpeedAcceleration(&result)) {
        LOG(ERROR) << "GenerateSpeedAcceleration fail";
        return false;
      }
    }
  }
  ...
  LOG(INFO) << "speed profile total time: " << diff.count() << " ms.";
  return true;
}
...
bool HybridAStar::Plan(
  ...
  if (!ValidityCheck(start_node_)) {
    LOG(ERROR) << "start_node in collision with obstacles";
    return false;
  }
  if (!ValidityCheck(end_node_)) {
    LOG(ERROR) << "end_node in collision with obstacles";
    return false;
  }
  auto const map_time = std::chrono::high_resolution_clock::now();
  grid_a_star_heuristic_generator_->GenerateDpMap(ex, ey, XYbounds_,
                                                  obstacles_linesegments_vec_);
  LOG(INFO) << "map time "
            << std::chrono::duration_cast<std::chrono::milliseconds>(
                   std::chrono::high_resolution_clock::now() - map_time)
                   .count()
            << "ms";
  // load open set, pq
  ...
  auto const astar_start_time = std::chrono::high_resolution_clock::now();
  ...
  LOG(INFO) << "heuristic time is " << heuristic_time << "ms.";
  LOG(INFO) << "reed shepp time is " << rs_time << "ms.";
