...
std::string STBoundary::TypeName(BoundaryType type) {
  ...
  LOG(WARNING) << "Unknown boundary type " << static_cast<int>(type)
               << ", treated as UNKNOWN";
  return "UNKNOWN";
}
...
bool STBoundary::GetUnblockSRange(const double curr_time, double* s_upper,
  ...
  *s_lower = 0.0;
  ...
  if (!GetIndexRange(lower_points_, curr_time, &left, &right)) {
    LOG(ERROR) << "Fail to get index range.";
    return false;
  }
  ...
  if (boundary_type_ == BoundaryType::STOP ||
    ...
    *s_lower = std::fmax(*s_lower, upper_cross_s);
    ...
    LOG(INFO) << "boundary_type is not supported. boundary_type: "
...
bool STBoundary::IsPointInBoundary(const STPoint& st_point) const {
  ...
  if (!GetIndexRange(lower_points_, st_point.t(), &left, &right)) {
    LOG(ERROR) << "failed to get index range.";
    return false;
  }
  const double check_upper =
      CrossProd(st_point, upper_points_[left], upper_points_[right]);
  const double check_lower =
      CrossProd(st_point, lower_points_[left], lower_points_[right]);
...
STBoundary STBoundary::ExpandByT(const double t) const {
  if (lower_points_.empty()) {
    LOG(ERROR) << "The current st_boundary has NO points.";
    return STBoundary();
  }
...
bool STBoundary::IsValid(
  ...
  if (point_pairs.size() < 2) {
    LOG(ERROR)
        << "point_pairs.size() must >= 2, but current point_pairs.size() = "
        << point_pairs.size();
    return false;
  }
  ...
  for (size_t i = 0; i < point_pairs.size(); ++i) {
    ...
    if (curr_upper.s() < curr_lower.s()) {
      LOG(ERROR) << "ST-boundary's upper-s must >= lower-s";
      return false;
    }
    if (std::fabs(curr_lower.t() - curr_upper.t()) > kStBoundaryEpsilon) {
      LOG(ERROR) << "Points in every ST-point pair should be at the same time.";
      return false;
    }
    if (i + 1 != point_pairs.size()) {
      ...
      if (std::fmax(curr_lower.t(), curr_upper.t()) + kMinDeltaT >=
        ...
        LOG(ERROR) << "Latter points should have larger t: "
...
bool STBoundary::GetIndexRange(const std::vector<STPoint>& points,
  ...
  if (t < points.front().t() || t > points.back().t()) {
    LOG(ERROR) << "t is out of range. t = " << t;
    return false;
  }
