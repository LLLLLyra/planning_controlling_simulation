...
IterativeAnchoringSmoother::IterativeAnchoringSmoother(
  ...
  const auto& vehicle_param = vehicle::VehicleParam();
...
bool IterativeAnchoringSmoother::Smooth(
  ...
  if (interpolated_size < 4) {
    ...
    LOG(INFO) << "interpolated_warm_start_path smaller than 4, can't enforce "
                 "initial zero kappa";
  ...
  if (FLAGS_enable_fem_pos_smoother) {
    if (!SmoothPath(interpolated_warm_start_path, bounds,
                    &smoothed_path_points)) {
      return false;
    }
  } else {
    if (!SmoothPath(interpolated_warm_start_path, &smoothed_path_points)) {
      return false;
    }
  }
  ...
  LOG(INFO) << "iterative anchoring path smoother time: "
  ...
  LOG(INFO) << "iterative anchoring speed smoother time: "
  ...
  LOG(INFO) << "iterative anchoring smoother total time: "
            << diff.count() * 1000.0 << " ms.";

  LOG(INFO) << "discretized_trajectory size " << discretized_trajectory->size();
  return true;
}
...
bool IterativeAnchoringSmoother::ReAnchoring(
  ...
  if (colliding_point_index.empty()) {
    LOG(INFO) << "no point needs to be re-anchored";
    return true;
  }
...
bool IterativeAnchoringSmoother::SmoothPath(
  ...
  while (!is_collision_free) {
    ...
    LOG(INFO) << "loop iteration number is " << counter;
    ++counter;
  }
  return true;
}
...
bool IterativeAnchoringSmoother::SmoothPath(
  ...
  for (const auto& raw_path_point : raw_path_points) {
    xpoints.push_back(raw_path_point.x());
    ypoints.push_back(raw_path_point.y());
  }
  const int k = planner_open_space_config_.iterative_anchoring_smoother_config()
                    .bspline_config()
                    .degrees();
  const int num = raw_path_points.size();
  std::vector<double> t;
  ...
  for (double s = 0.0; s <= num - k; s += kDenseStep) {
    smoothed_point2d.push_back({spl_x.bspline(s), spl_y.bspline(s)});
  }
  smoothed_point2d.push_back({spl_x.bspline(num - k - kMathEpsilon),
    ...
    for (const auto& obstacle_linesegments : obstacles_linesegments_vec_) {
      for (const LineSegment2d& linesegment : obstacle_linesegments) {
        if (ego_box.HasOverlap(linesegment)) {
          ...
          LOG(INFO) << "point at " << i << "collied with LineSegment "
...
bool IterativeAnchoringSmoother::SmoothSpeed(const double init_a,
  ...
  const double sEpislon = 1.0e-2;
  for (size_t i = 1; i < num_of_knots; ++i) {
    if (s[i - 1] - s[i] > kEpislon) {
      LOG(ERROR) << "unexpected decreasing s in speed smoothing at time "
...
bool IterativeAnchoringSmoother::CombinePathAndSpeed(
  ...
  LOG(INFO) << "speed_points.TotalTime() " << speed_points.TotalTime();
  for (double cur_rel_time = 0.0; cur_rel_time < time_horizon;
    ...
    points::PathPoint path_point = path_points.Evaluate(speed_point.s());

    points::TrajectoryPoint trajectory_point;
  ...
  LOG(INFO) << "path length before combine " << path_points.Length();
  LOG(INFO) << "trajectory length after combine "
...
bool IterativeAnchoringSmoother::GenerateStopProfileFromPolynomial(
  ...
  LOG(ERROR) << "GenerateStopProfileFromPolynomial fails.";
  return false;
}
