...
std::vector<Condition> EndConditionSampler::SampleLatEndConditions() const {
  ...
  std::array<double, 3> end_d_candidates = {0.0, -0.2, 0.2};
  std::array<double, 4> end_s_candidates = {1, 2, 5, 10};
...
EndConditionSampler::QueryPathTimeObstacleSamplePoints() const {
  const auto& vehicle_config = vehicle::VehicleParam();
...
void EndConditionSampler::QueryFollowPathTimePoints(
    const vehicle::VehicleParam& vehicle_config, const std::string& obstacle_id,
  ...
  for (const auto& path_time_point : follow_path_time_points) {
    ...
    double s_upper =
        path_time_point.s() - vehicle_config.front_edge_to_center();
