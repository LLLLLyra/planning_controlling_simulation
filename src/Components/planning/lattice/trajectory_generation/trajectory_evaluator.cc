...
#include "planning/common/constraint_checker1d.h"
#include "planning/common/piecewise_acceleration_trajectory1d.h"
...
TrajectoryEvaluator::TrajectoryEvaluator(
  ...
  LOG(INFO) << "Number of valid 1d trajectory pairs: " << cost_queue_.size();
}
...
double TrajectoryEvaluator::CentripetalAccelerationCost(
  ...
  for (double t = 0.0; t < FLAGS_trajectory_time_length;
    ...
    points::PathPoint ref_point = PathMatcher::MatchToPath(*reference_line_, s);
...
bool TrajectoryEvaluator::InterpolateDenseStPoints(
  ...
  if (t < st_points[0].t() || t > st_points[st_points.size() - 1].t()) {
    LOG(INFO) << "AutoTuning InterpolateDenseStPoints Error";
    return false;
  }
