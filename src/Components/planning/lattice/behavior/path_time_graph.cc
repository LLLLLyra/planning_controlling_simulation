...
boundary::SLBoundary PathTimeGraph::ComputeObstacleBoundary(
  ...
  boundary::SLBoundary sl_boundary;
...
void PathTimeGraph::SetupObstacles(
  ...
  std::sort(
      ...
      [](const boundary::SLBoundary& sl0, const boundary::SLBoundary& sl1) {
...
void PathTimeGraph::SetStaticObstacle(
  ...
  if (sl_boundary.start_s() > path_range_.second ||
    ...
    LOG(INFO) << "Obstacle [" << obstacle_id << "] is out of range.";
    return;
  }
  ...
  LOG(INFO) << "ST-Graph mapping static obstacle: " << obstacle_id
...
void PathTimeGraph::SetDynamicObstacle(
  ...
  while (relative_time < time_range_.second) {
    points::TrajectoryPoint point = obstacle->GetPointAtTime(relative_time);
...
std::vector<std::pair<double, double>> PathTimeGraph::GetLateralBounds(
  ...
  for (const boundary::SLBoundary& static_sl_boundary :
