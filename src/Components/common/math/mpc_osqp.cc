...
MpcOsqp::MpcOsqp(const Eigen::MatrixXd &matrix_a,
  ...
  LOG(INFO) << "state_dim" << state_dim_;
  LOG(INFO) << "control_dim_" << control_dim_;
...
void MpcOsqp::CalculateEqualityConstraint(std::vector<c_float> *A_data,
  ...
  LOG(INFO) << "matrix_constraint with A";
  LOG(INFO) << matrix_constraint;
  ...
  LOG(INFO) << "matrix_constraint with B";
  LOG(INFO) << matrix_constraint;
  ...
  LOG(INFO) << "matrix_constraint with I";
  LOG(INFO) << matrix_constraint;
...
OSQPData *MpcOsqp::Data() {
  ...
  if (data == nullptr) {
    ...
    std::vector<c_int> P_indices;
    std::vector<c_int> P_indptr;
    LOG(INFO) << "before CalculateKernel";
    CalculateKernel(&P_data, &P_indices, &P_indptr);
    LOG(INFO) << "CalculateKernel done";
    data->P =
        csc_matrix(kernel_dim, kernel_dim, P_data.size(), CopyData(P_data),
                   CopyData(P_indices), CopyData(P_indptr));
    LOG(INFO) << "Get P matrix";
    data->q = gradient_.data();
    LOG(INFO) << "before CalculateEqualityConstraint";
    std::vector<c_float> A_data;
    std::vector<c_int> A_indices;
    std::vector<c_int> A_indptr;
    CalculateEqualityConstraint(&A_data, &A_indices, &A_indptr);
    LOG(INFO) << "CalculateEqualityConstraint done";
    data->A =
...
bool MpcOsqp::Solve(std::vector<double> *control_cmd) {
  ...
  CalculateGradient();
  LOG(INFO) << "After Calc Gradient";
  CalculateConstraintVectors();
  LOG(INFO) << "MPC2Matrix";

  OSQPData *data = Data();
  LOG(INFO) << "OSQP data done";
  LOG(INFO) << "OSQP data n" << data->n;
  LOG(INFO) << "OSQP data m" << data->m;
  for (int i = 0; i < data->n; ++i) {
    LOG(INFO) << "OSQP data q" << i << ":" << (data->q)[i];
  }
  LOG(INFO) << "OSQP data l" << data->l;
  for (int i = 0; i < data->m; ++i) {
    LOG(INFO) << "OSQP data l" << i << ":" << (data->l)[i];
  }
  LOG(INFO) << "OSQP data u" << data->u;
  for (int i = 0; i < data->m; ++i) {
    LOG(INFO) << "OSQP data u" << i << ":" << (data->u)[i];
  }

  OSQPSettings *settings = Settings();
  LOG(INFO) << "OSQP setting done";
  OSQPWorkspace *osqp_workspace = nullptr;
  osqp_setup(&osqp_workspace, data, settings);
  // osqp_workspace = osqp_setup(data, settings);
  LOG(INFO) << "OSQP workspace ready";
  ...
  for (size_t i = 0; i < control_dim_; ++i) {
    ...
    LOG(INFO) << "control_cmd:" << i << ":" << control_cmd->at(i);
  }
