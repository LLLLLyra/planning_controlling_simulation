...
points::PathPoint PathMatcher::MatchToPath(
    const std::vector<points::PathPoint>& reference_line, const double x,
    const double y) {
  CHECK_GT(reference_line.size(), 0U);

  auto func_distance_square = [](const points::PathPoint& point, const double x,
                                 const double y) {
    double dx = point.x() - x;
    double dy = point.y() - y;
    return dx * dx + dy * dy;
...
points::PathPoint PathMatcher::MatchToPath(
  ...
  auto comp = [](const points::PathPoint& point, const double s) {
    return point.s() < s;
  };
...
points::PathPoint PathMatcher::FindProjectionPoint(const points::PathPoint& p0,
