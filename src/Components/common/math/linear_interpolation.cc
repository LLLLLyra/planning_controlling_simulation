...
points::TrajectoryPoint InterpolateUsingLinearApproximation(
  ...
  if (!tp0.has_path_point() || !tp1.has_path_point()) {
    points::TrajectoryPoint p;
    p.mutable_path_point()->CopyFrom(points::PathPoint());
    return p;
  }
  const points::PathPoint pp0 = tp0.path_point();
  const points::PathPoint pp1 = tp1.path_point();
  double t0 = tp0.relative_time();
  double t1 = tp1.relative_time();

  points::TrajectoryPoint tp;
  tp.set_v(lerp(tp0.v(), t0, tp1.v(), t1, t));
  tp.set_a(lerp(tp0.a(), t0, tp1.a(), t1, t));
  tp.set_relative_time(t);
  tp.set_steer(slerp(tp0.steer(), t0, tp1.steer(), t1, t));

  points::PathPoint *path_point = tp.mutable_path_point();
